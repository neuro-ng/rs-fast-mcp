...
use rs_fast_mcp::mcp::types::{JsonRpcRequest, RequestId, JsonRpcResponse};
...
async fn test_server_mounting() {
    ...
    let server_path = std::path::Path::new(&manifest_dir)
        .join("target/debug/examples/simple_server");
    
    if !server_path.exists() {
        eprintln!("simple_server binary not found at {:?}. Run `cargo build --examples` first.", server_path);
    }

    // 2. Create StdioTransport
    let transport = StdioClientTransport::new_process(server_path.to_str().unwrap(), &[]).expect("Failed to create transport");
    ...
    println!("Mounted tools: {:?}", tools.iter().map(|t| &t.name).collect::<Vec<_>>());
    
    // Check for prefixed names
    let echo_tool = host.get_tool("remote_echo");
    assert!(echo_tool.is_some(), "Tool remote_echo not found. Available: {:?}", tools.iter().map(|t| &t.name).collect::<Vec<_>>());
    ...
    let structured = result.get("structured_content").expect("Missing structured_content");
    ...
    println!("Mounted resources: {:?}", resources.iter().map(|r| &r.uri).collect::<Vec<_>>());
    
    assert!(!resources.is_empty(), "No resources found");
    assert!(resources.iter().any(|r| r.uri == "file:///hello"), "Resource file:///hello not found");
    ...
    let read_resp: JsonRpcResponse = host.handle_request(read_req).await.expect("Read Request failed");
    ...
    println!("Read Resource: {}", serde_json::to_string_pretty(&read_res_content).unwrap());
    
    let contents = read_res_content.get("contents").expect("Missing contents").as_array().expect("Contents not array");
    ...
    println!("Mounted prompts: {:?}", prompts.iter().map(|p| &p.name).collect::<Vec<_>>());
    
    // Expect "remote_greet"
    assert!(prompts.iter().any(|p| p.name == "remote_greet"), "Prompt remote_greet not found");
    
    // 10. Get Prompt
    let prompt = host.get_prompt("remote_greet").expect("Failed to get prompt");
    ...
    let prompt_resp: JsonRpcResponse = host.handle_request(prompt_req).await.expect("Prompt Request failed");
    println!("Prompt Exec Response: {}", serde_json::to_string_pretty(&prompt_resp.result).unwrap());
    
    let messages = prompt_resp.result.get("messages").expect("Missing messages").as_array().expect("Messages not array");
    ...
    // { "type": "text", "text": "..." }
    let text = content.get("text").expect("Missing text").as_str().expect("Text not string");
