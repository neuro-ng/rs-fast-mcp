...
impl Transport for MockTransport {
    async fn start(
        ...
        _notification_rx: Option<
            tokio::sync::broadcast::Receiver<rs_fast_mcp::mcp::types::JsonRpcMessage>,
        >,
    ) -> Result<(), FastMCPError> {
...
async fn test_lifespan_hooks() {
    ...
    assert!(result.is_ok());
    assert!(
        *startup_called.lock().unwrap(),
        "Startup hook should be called"
    );
    assert!(
        *shutdown_called.lock().unwrap(),
        "Shutdown hook should be called"
    );
}
