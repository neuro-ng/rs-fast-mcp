use crate::atproto::types::{Session, FeedItem};
use std::sync::Arc;
...
impl Client {
    ...
    pub async fn login(&self, identifier: &str, password: &str) -> Result<String, String> {
        let url = self.base_url.join("/xrpc/com.atproto.server.createSession").unwrap();
    ...
    pub async fn get_timeline(&self) -> Result<Vec<FeedItem>, String> {
        ...
        let url = self.base_url.join("/xrpc/app.bsky.feed.getTimeline").unwrap();
    ...
    pub async fn create_record(&self, collection: &str, record: serde_json::Value) -> Result<String, String> {
         let token = self.get_token().await?;
         let guard = self.session.read().await;
         let repo = guard.as_ref().unwrap().did.clone();

         let url = self.base_url.join("/xrpc/com.atproto.repo.createRecord").unwrap();
