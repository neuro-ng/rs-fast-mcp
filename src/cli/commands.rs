...
pub async fn run(
    ...
    let config_path = config_path.map(std::path::Path::new).unwrap_or_else(|| {
        std::path::Path::new("fastmcp.json")
    });
        ...
        match crate::mcp::config::ServerConfig::load_from_file(config_path) {
            Ok(config) => {
                ...
                for (name, tool_config) in config.tools {
                    match tool_config {
                        crate::mcp::config::ToolConfig::Command { command, args, env, description } => {
                             ...
                             let tool = crate::tools::tool::Tool {
                                ...
                                data: crate::tools::tool::ToolKind::Function(crate::tools::tool::ToolFunction {
                                    ...
                                    fn_handler: std::sync::Arc::new(Box::new(move |_ctx, args| {
                             ...
                             server.add_tool(tool).unwrap_or_else(|e| error!("Failed to add tool {}: {}", name, e));
                        }
                    }
                }
                ...
                for (name, resource_config) in config.resources {
                     match resource_config {
                        crate::mcp::config::ResourceConfig::File { path, mime_type } => {
                            ...
                            let handler = Box::new(move |_uri: String, _ctx: crate::server::context::Context| {
                                let p = file_path.clone();
                                Box::pin(async move {
                                     match tokio::fs::read_to_string(&p).await {
                                        Ok(content) => Ok(vec![crate::mcp::types::ResourceContents {
                                            uri: _uri,
                                            mime_type: None,
                                            text: Some(content),
                                            blob: None,
                                        }]),
                                        Err(e) => Err(crate::error::FastMCPError::Resource(crate::error::ErrorData {
                                            code: Some(1),
                                            message: e.to_string(),
                                            data: None,
                                        })),
                                     }
                                }) as std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<crate::mcp::types::ResourceContents>, crate::error::FastMCPError>> + Send>>
                            });
                            server.add_resource(resource, Some(std::sync::Arc::new(handler))).unwrap_or_else(|e| error!("Failed to add resource {}: {}", name, e));
                        }
...
pub async fn dev(
    ...
    let mut server_args = vec!["run".to_string(), "--transport".to_string(), "stdio".to_string()];
