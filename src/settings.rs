...
impl Settings {
    ...
    pub fn load() -> Self {
        ...
        if let Ok(val) = env::var("FASTMCP_INCLUDE_TAGS") {
            settings.include_tags = val.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
        }

        if let Ok(val) = env::var("FASTMCP_EXCLUDE_TAGS") {
            settings.exclude_tags = val.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
        }

        // Load secrets
        let secrets_dir = env::var("FASTMCP_SECRETS_DIR").unwrap_or_else(|_| "/run/secrets".to_string());
        let secrets_path = Path::new(&secrets_dir);

        if secrets_path.exists() && secrets_path.is_dir()
                        ...
                        && path.is_file()
                             && let Some(file_name) = path.file_name().and_then(|n| n.to_str())
                                 && let Ok(content) = fs::read_to_string(&path) {
                                     settings.secrets.insert(file_name.to_string(), content.trim().to_string());
                                 }
                }
            }
...
mod tests {
    ...
    fn test_load_secrets() {
        ...
        let settings = Settings::load();
        assert_eq!(settings.secrets.get("api_key").map(|s| s.as_str()), Some("super_secret_value"));
